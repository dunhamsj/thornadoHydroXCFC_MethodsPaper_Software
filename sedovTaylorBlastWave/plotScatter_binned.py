...
for j in range( len( field ) ) :
    ...
    for i in range( len( plotfileDirectory ) ) :
        ...
        d /= norm[j]
        nbins = 10
        r_edges   = np.linspace(0.0 , 1.0 , nbins + 1)
        r_centers = np.linspace(0.05, 0.95, nbins)
        means     = np.empty(nbins)
        stddevs   = np.empty(nbins)
        for k in range(nbins):
            ind_r = np.where((r >= r_edges[k]) & (r < r_edges[k+1]))[0]
            dd = d[ind_r]
            means  [k] = dd.mean()
            stddevs[k] = dd.std()
        ax.errorbar(r_centers, means, yerr = stddevs, \
                    label = mklabel[i], \
                    marker = '.', \
                    ls = 'none', \
                    lw = lw[i])
...
#                    rasterized = True )
    ...
    if ( saveFig ) :
        figName = gv.paperDirectory \
                    + 'Figures/fig.stbw_scatter_binned_{:}.pdf' \
                      .format( field[j] )
